				...
				if(mysqli_num_rows($sql) > 0){
					...
					while($data = mysqli_fetch_assoc($sql)){
						...
						<tr>
							...
							<td>'.$data['ukuran'].'</td>
							<td>
								<a href="index.php?page=edit&id_pemesanan='.$data['id_pemesanan'].'" class="btn btn-secondary btn-sm">Edit</a>
								<a href="delete.php?id_pemesanan='.$data['id_pemesanan'].'" class="btn btn-danger btn-sm" onclick="return confirm(\'Yakin ingin menghapus data ini?\')">Delete</a>
							</td>
						</tr>
						';
						$no++;
